(**
   ...
   Generalization from string to Alphabet: 
...
(* Subseq : SUBSEQUENCE with module A=A *)
 : TREE with module A=A =
  struct

  module A = A
  module Subseq = Subsequence(A)
...
(* type of suffix trees *)
  type t = {
    ...
    let label st node =
      ...
      else A.sub (get0 st node.seqid) node.start (length st node)
    ...
    let rec path st node =
      ...
      | Some parent -> A.concat (path st parent) (label st node)
...
(* test whether the extension of an implicit node by [seqar.(k).[i]] is still recognized in the GST,
   and if yes, returns the implicit node extended by 1 position, otherwise returns [None]. *)
    let has_child st (explicit,implicit,child) (k,i) =
      let a = Subseq.length implicit in
      if a <> 0 then
  (* NB. if eq_char (get0 st child.seqid).[child.start+a] (get0 st k).[i] *)
  if eq_char (A.get (get0 st child.seqid) (child.start+a)) (A.get (get0 st k) i)
...
(* test whether the extension of an implicit node by '\000' exists in the GST,
   and if so, returns the corresponding leaves, otherwise returns None. *)
    let has_end st (explicit,implicit,child) =
      ...
      let a = A.length implicit in
      if a <> 0 then
  (* if (get0 st child.seqid).[child.start+a] = '\000' *)
  if A.get (get0 st child.seqid) (child.start+a) = A.dummy
	then Some [child]
	else None
      else
	match explicit.v with
	| I x -> Some (Hashtbl.find_all x.children A.dummy (* NB. '\000' *) )
	| L _ -> None
...
(* --------------------------------
   ...
   -------------------------------- *)
    ...
    let add_leaf st node seqid start final_ref index =
	  ...
	  Hashtbl.add x.children (A.get (get0 st seqid) start) child
...
(* make explicit an implicit node by inserting a new node between [explicit] and [child] *)
	...
	| I x ->
		...
		x.locals <- Ext.add strid x.locals end;
	      ...
	      let c_child_old = A.get (get0 st child.seqid) child.start in
	      let c_child_new = A.get (get0 st child.seqid) (child.start+a) in
		...
		v = I
	      ...
	      set_visible st n' (Visible.get_visible (path st n'));
	      n' end
...
(*	mutable new_maximal : node LSet.t; *)
    ...
    (* NB. if (get0 st strid).[res.startj] <> '\000' then begin *)
    if A.get (get0 st strid) res.startj <> A.dummy then begin
...
*)
    ...
    let rec find_factor st str =
      ...
      let (explicit, (s,i,len), child) = find_factor_aux st st.root (str,0,A.length str) in
      (explicit, A.sub s i len, child)
	...
	let l = !(child.final) - child.start + 1 in
	let a = ref 1 in
  (* NB. while !a < l & !a < w & eq_char (get0 st child.seqid).[child.start + !a] (Subseq.get implicit !a) do *)
  while !a < l & !a < w & eq_char (A.get (get0 st child.seqid) (child.start + !a)) (Subseq.get implicit !a) do
	  incr a
...
(* add a string and returns its strid. If the string already exists, the GST is not modified,
  ...
  let add st str =
	...
	let str0 = A.append str A.dummy (* NB. str ^ String.make 1 '\000' *) in (* add a terminal symbol *)
	...
	for pos = 0 to A.length (* NB. String.length *) str0 - 1 do (* NB. for every position [i] in the sequence ... *)
...
(* readable version of a GST *)
	...
	(fun l n ->
	  ...
	  then Node ("", maximal0 n, ext st n, l)
	    ...
	    let w = A.to_string (label st n) in
	    if l=[]
